   ...
   if (!noData)
      ...
      for (int i = 0; i < temp_values.rows; i++)
         if (temp_values[i][0] == _NAN_)
            {
            warning("PDFLineChart::Non-numeric x-input value read at line %d %lf, Some data points won't be drawn", i, temp_values[i][0]);
...
void PDFLineChart::SetDataValues(Vector & x_values, Matrix & y_values)
   {
   // check that allocated storage has the right dimensions

   // We can add or remove data points as necessary
   if (y_values.cols != yValues.cols && y_values.cols <= x_values.Length())
      {
      yValues.Dimension(yValues.rows, y_values.cols);
      xValues.Dimension(1, y_values.cols);
      }

   // But we won't create new dataseries unless the users asks us to
   if (y_values.rows != yValues.rows)
      error("PDFLineChart::SetDataValues called with incorrect number of data series");

   // One x-coordinate needed for each datapoint
   if (y_values.cols != yValues.cols || x_values.Length() < xValues.cols)
      error("PDFLineChart::SetDataValues called with incorrect number of x coordinates");
   ...
   if (!noData)
      for (int i = 0; i < x_values.dim; i++)
         if (x_values[i] == _NAN_)
            {
            warning("PDFLineChart::Missing x-value argument supplied, some data points won't be drawn");
            break;
            }
...
void PDFLineChart::SetDataValue(int series, int datapt, double value)
   ...
   if (series >= 0 && series < yValues.rows)
      if (datapt < yValues.cols)
         ...
         error("PDFLineChart:: Cannot set data value. Datapt argument is out of range");
   else
...
bool PDFLineChart::OpenChart(PDF & pdf)
   {
   InitializePage(pdf);
   keepLegend = true;
   ...
   for (int i = 0; i < legend.numSeries; i++)
      {
      if (useColor)
         lines[i].InitializeStyle(0);
      else
         lines[i].InitializeStyle(i);
      }
...
void PDFLineChart::SetOtherDimensions(PDF & pdf)
   {
   PDFChartBasics::SetOtherDimensions(pdf);
...
void PDFLineChart::CloseChart(PDF & pdf)
   {
   for (int i = 0; i < yValues.rows; i++)
