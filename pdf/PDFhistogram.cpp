...
void PDFHistogram::SetDataCounts(const Matrix & data_counts)
   ...
   Matrix new_counts;
   new_counts.Dimension(data_counts.rows - 2, data_counts.cols);
...
bool PDFHistogram::OpenChart(PDF & pdf)
   {
   if (useBinData)
...
void PDFHistogram::InitializeAxes()
   {
   if (!useBinData)
      xAxis.GetDataRange(dataValues);
   else
      xAxis.GetDataRange(xRanges);
   ...
   Matrix count_values;
   count_values.Dimension(stacked ? 1 : dataValues.rows , numBars);
   count_values.Zero();
...
void PDFHistogram::AllocateBarMatrix(int num_series, int num_bars_per_series)
   ...
   barSeries = num_series;  // track this separately because dataValues.rows
                            // will change, then we calculate the number of bars
                            // per series, then we re-allocate the bar matrix ->
                            // requires dimensions of previous matrix
   }
...
void PDFHistogram::ResizeBarMatrix(int new_series, int new_bars_per_series)
   ...
   for (int i = 0; i < new_series; i++)
      if (new_bars_per_series > 0)
         new_bars[i] = new PDFChartBar[new_bars_per_series];
      else
         new_bars[i] = NULL;
...
void PDFHistogram::DrawBarsStacked(PDF & pdf)
   ...
   for (int k = 0; k < dataValues.rows; k++)
      ...
      for (int i = 0; i < numBars; i++)
         {
         if (useColor)
            lines[k].SelectColorBrush(pdf);
         else
            lines[k].SelectGrayBrush(pdf);
...
void PDFHistogram::DrawBarsAligned(PDF & pdf)
   ...
   for (int k = 0; k < dataValues.rows; k++)
      ...
      for (int i = 0; i < numBars; i++)
         ...
         if (bars[k][i].hasData)
            {
            pdf.page.FillRectangle(MapX(bar_x0 + 100 * xAxis.Eps()), MapY(0.0),
               MapX(bar_x1 - 100 * xAxis.Eps()), MapY(bars[k][i].count));
            pdf.page.DrawRectangle(MapX(bar_x0 + 100 * xAxis.Eps()), MapY(0.0),
               MapX(bar_x1 - 100 * xAxis.Eps()), MapY(bars[k][i].count));
            }
...
void PDFHistogram::SetCategoricalBars(bool categorical, double first_bar_category, double last_bar_category)
   ...
   if (categorical)
      ...
      if (last_bar_category != _NAN_)
         {
         SetBarEnd(last_bar_category);
         xAxis.minMax = last_bar_category + 0.5;
         }
      else // let data determine bar end
         {
         SetBarEnd(_NAN_);
         xAxis.minMax = _NAN_;
         }

      if (first_bar_category != _NAN_)
         {
         SetBarStart(first_bar_category - 0.5);
         xAxis.SetStepStart(first_bar_category - 0.5);
         xAxis.maxMin = first_bar_category - 0.5;
         }
      else
         {
         SetBarStart(_NAN_);
